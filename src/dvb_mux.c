 ...
 * to generate SI tables but the vbi3_encode_dvb_pdc_descriptor() function
...
/* Preliminary. */
enum {
	VBI3_ERR_BUFFER_OVERFLOW = 0x7081800,
	VBI3_ERR_RAW_BUFFER_OVERFLOW,
	VBI3_ERR_LINE_NUMBER,
	VBI3_ERR_LINE_ORDER,
	VBI3_ERR_INVALID_SERVICE,
	VBI3_ERR_SAMPLE_NUMBER,
	VBI3_ERR_AMBIGUOUS_VIDEOSTD,
	VBI3_ERR_RAW_DATA_INTERRUPTION,
	VBI3_ERR_NO_RAW_DATA,
	VBI3_ERR_NO_SLICED_DATA,
	VBI3_ERR_SAMPLING_PAR,
};
...
/* vbi3_sampling_par->offset just counts samples since 0H. */
 ...
 *   the offending vbi3_sliced structure.
 * @param s_left The number of vbi3_sliced structures in the @a sliced
 *   array.
 ...
 *   A vbi3_sliced structure contains a line number outside the valid
...
insert_sliced_data_units	(uint8_t **		packet,
				 ...
				 vbi3_bool		fixed_length)
{
	static const vbi3_bool strict = TRUE;
	uint8_t *p;
	const vbi3_sliced *s;
	...
	*last_du_size = 0;
	...
	for (; s_left > 0; ++s, --s_left) {
		...
		case VBI3_SLICED_VPS | VBI3_SLICED_VPS_F2:
		...
		bad_service:
			...
			*sliced = s;
			...
			return VBI3_ERR_INVALID_SERVICE;
		}
 ...
 *   On failure it will point at the offending vbi3_sliced structure.
 * @param sliced_left @a *sliced_left shall contain the number of
 *   vbi3_sliced structures in the @a sliced array, or it can be
				 ...
				 vbi3_bool		stuffing)
{
	...
	if (unlikely (NULL == p || p_left < 2)) {
		/* errno = VBI3_ERR_BUFFER_OVERFLOW; */
		return FALSE;
	}
				 ...
				 vbi3_bool		stuffing)
{
	...
	if (videostd_set & VBI3_VIDEOSTD_SET_525_60) {
		if (unlikely (videostd_set & VBI3_VIDEOSTD_SET_625_50))
			return VBI3_ERR_AMBIGUOUS_VIDEOSTD;
		f2_start = 263;
	} else if (likely (videostd_set & VBI3_VIDEOSTD_SET_625_50)) {
		f2_start = 313;
	} else {
		return VBI3_ERR_AMBIGUOUS_VIDEOSTD;
	}		
...
struct _vbi3_dvb_mux {
	...
	/* The vbi3_sliced.line number used to encode the
	   unfinished raw VBI line. */
	unsigned int		raw_line;

	/* The vbi3_sampling_par.offset used to encode the
	   unfinished raw VBI line. */
	int			raw_offset;

	/* The vbi3_sampling_par.samples_per_line used to
 ...
 *   vbi3_dvb_pes_mux_new() or vbi3_dvb_ts_mux_new().
 *
 * Clears the internal buffers of the DVB VBI multiplexer.
 *
 * After a vbi3_dvb_mux_reset() call the vbi3_dvb_mux_cor() function
				 ...
				 unsigned int		line)
{
	...
	if (unlikely (NULL == raw))
		return VBI3_ERR_NO_RAW_DATA;

	if (unlikely (NULL == sp))
		return VBI3_ERR_SAMPLING_PAR;
	...
	if (unlikely (line < (unsigned int) sp->start[field]))
		return VBI3_ERR_RAW_BUFFER_OVERFLOW;
...
static vbi3_bool
valid_sampling_par		(vbi3_dvb_mux *		mx,
				 const vbi3_sampling_par *sp)
{
...
#if 3 == VBI_VERSION_MINOR
	if (unlikely (0 == (sp->videostd_set & VBI3_VIDEOSTD_SET_625_50)))
		return FALSE;
	...
	if (unlikely (VBI3_PIXFMT_YUV420 != sp->sp_sample_format))
		return FALSE;
#endif
	...
	return _vbi3_sampling_par_valid_log (sp, &mx->log);
}

static void
init_pes_packet_header		(vbi3_dvb_mux *		mx)
{
 ...
 *   vbi3_sliced structures in the @a sliced array. It will
 ...
 *     @c VBI3_VIDEOSTD_SET_625_50, in libzvbi 0.2.x .scanning must be 625.
 *   - .sampling_format must be @c VBI3_PIXFMT_Y8 or
 *     @c VBI3_PIXFMT_YUV420. Chrominance samples are ignored.
 ...
 *   of a raw VBI line, @a *sliced will point at the vbi3_sliced
 ...
 *   in the @a raw buffer).
 *
 * On all errors @a *sliced points at the offending vbi3_sliced
 * structure and the output buffer contents are undefined.
 */
static int
generate_pes_packet		(vbi3_dvb_mux *		mx,
				 ...
				 int64_t		pts)
{
	...
	for (;;) {
		...
		if (0 == (service_mask & VBI3_SLICED_VBI_625)) {
			s_begin = ++s;
			continue;
		}
...
static void
generate_ts_packet_header	(vbi3_dvb_mux *		mx,
				 unsigned int		offset)
{
	uint8_t *p;
 ...
 *   vbi3_dvb_mux_set_pes_packet_size(), is too small to contain all
 ...
 *   in the @a raw buffer).
 *
 * On all errors @a *sliced will point at the offending vbi3_sliced
 ...
 * @param sliced_lines The number of vbi3_sliced structures
 ...
 *   must be valid, with the following additional constraints:
 *   - .videostd_set must contain one or more bits from the
 *     @c VBI3_VIDEOSTD_SET_625_50. In libzvbi 0.2.x .scanning must be
 *     625.
 *   - .sampling_format must be @c VBI3_PIXFMT_Y8 or
 *     @c VBI3_PIXFMT_YUV420. Chrominance samples are ignored.
 *   - .sampling_rate must be @c 13500000.
 ...
 * vbi3_dvb_pes_mux_new() or vbi3_dvb_ts_mux_new() once for each
 * PES or TS packet.
 *
 * @returns
 * @c FALSE on failure.
 * - No callback function was selected with vbi3_dvb_pes_mux_new()
 *   or vbi3_dvb_ts_mux_new().
 ...
 * the data is larger than the maximum size the vbi3_dvb_mux_feed() and
 * vbi3_dvb_mux_cor() functions will fail.
 *
...
/**
 * @param mx DVB VBI multiplexer context allocated with
 *   vbi3_dvb_pes_mux_new() or vbi3_dvb_ts_mux_new(). Can be @c NULL.
 *
...
/**
 * @param callback Function to be called by vbi3_dvb_mux_feed() when
 *   a new packet is available. Can be @c NULL if you want to use the
 *   vbi3_dvb_mux_cor() coroutine instead.
 ...
 * freed with vbi3_dvb_mux_delete() when done. @c NULL on failure
 * (out of memory).
 *
 * @since 0.2.26
 */
				 ...
				 void *			user_data)
{
	...
	mx->packet = vbi3_malloc (4 + mx->max_packet_size);
				 ...
				 void *			user_data)
{
	...
	if (pid <= 0x000F || pid >= 0x1FFF) {
		/* errno = VBI3_ERR_INVALID_PID; */
		return NULL;
	}

	mx = vbi3_dvb_pes_mux_new (callback, user_data);
