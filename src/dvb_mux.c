...
/* $Id: dvb_mux.c,v 1.6.2.2 2006-05-07 06:04:58 mschimek Exp $ */
				 ...
				 unsigned int		offset)
{
	...
	if (videostd_set & VBI3_VIDEOSTD_SET_525_60) {
		if (videostd_set & VBI3_VIDEOSTD_SET_625_50) {
			fprintf (stderr,
...
static vbi3_bool
ts_packet_output		(vbi3_dvb_mux *		mx,
				 const uint8_t *	end)
{
	uint8_t *p;
				 ...
				 int64_t		pts)
{
	...
	while (sliced_size > 0) {
		...
		while (left > 0) {
			_vbi3_dvb_multiplex_sliced (&p, &left,
						   &sliced, &sliced_size,
						   mx->data_identifier,
						   service_set);
		}
...
/* XXX we're wasting a lot of space here on stuffing bytes.
   packet_size == 0 "minimum" would be nice for a file format. */
				 ...
				 void *			user_data)
{
	...
	mx = _vbi3_dvb_mux_pes_new (data_identifier,
				   packet_size,
				   videostd_set,
				   callback,
				   user_data);
