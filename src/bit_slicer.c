...
static vbi_bool								\
bit_slicer_##fmt		(vbi_bit_slicer *	bs,		\
				 uint8_t *		buf,		\
				 const uint8_t *	raw)		\
{									\
	unsigned int i, j, k;						\
...
/**
 * @param bs Pointer to a vbi_bit_slicer object allocated with
 *   vbi_bit_slicer_new(), can be NULL.
 *
 ...
 *   line later passed to vbi_bit_slicer_slice(). This limits the number of
 ...
 *   will be stored in the vbi_bit_slicer_slice() output. If this number
 ...
 * vbi_bit_slicer_slice(). This is a low level function, see also
 * vbi_raw_decoder_new().
 *
 * @returns
 * NULL when out of memory or parameters are invalid (e. g.
 * samples_per_line too small to contain CRI, FRC and payload).
 * Otherwise a pointer to an opaque vbi_bit_slicer object, which
 * must be deleted with vbi_bit_slicer_delete() when done.
 */
vbi_bit_slicer *
