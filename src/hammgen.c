...
/* $Id: hammgen.c,v 1.1.2.2 2008-02-27 07:57:15 mschimek Exp $ */
...
static uint8_t _vbi3_bit_reverse [256];
static uint8_t _vbi3_hamm8_fwd [16];
static int8_t _vbi3_hamm8_inv [256];
static uint8_t _vbi3_hamm24_fwd_0 [256];
static uint8_t _vbi3_hamm24_fwd_1 [256];
static uint8_t _vbi3_hamm24_fwd_2 [4];

/* Should be uint8_t, is int8_t for compatibility with earlier
   versions of libzvbi. */
static int8_t _vbi3_hamm24_inv_par [3][256];

static uint8_t _vbi3_hamm24_inv_d1_d4 [64];
static int32_t _vbi3_hamm24_inv_err [64];
...
generate_hamm24_inv_tables	(void)
{
	...
	/* D1_D4 = _vbi3_hamm24_inv_d1_d4[Byte_0 >> 2];
	   D5_D11 = Byte_1 & 0x7F;
	   D12_D18 = Byte_2 & 0x7F;
	   d = D1_D4 | (D5_D11 << 4) | (D12_D18 << 11);
	   ABCDEF = (  _vbi3_hamm24_inv_par[0][Byte_0]
	             ^ _vbi3_hamm24_inv_par[1][Byte_1]
	             ^ _vbi3_hamm24_inv_par[2][Byte_2]);
	   // Correct single bit error, set bit 31 on double bit error.
	   d ^= _vbi3_hamm24_inv_err[ABCDEF];
	...
	for (i = 0; i < 256; ++i) {
		...
		_vbi3_hamm24_inv_d1_d4 [i >> 2] = (+ (D1 << 0)
						  + (D2 << 1)
						  + (D3 << 2)
						  + (D4 << 3));
		...
		_vbi3_hamm24_inv_par [2][i] ^= 0x3F;
	}

	for (i = 0; i < 64; ++i) {
		unsigned int ii;

		/* Undo the ^ 0x3F in _vbi3_hamm24_inv_par[2][]. */
		ii = i ^ 0x3F;

		if (0 == ii) {
			/* No errors. */
			_vbi3_hamm24_inv_err[ii] = 0;
		} else if (0 == (ii & 0x1F) && 0x20 == (ii & 0x20)) {
			/* Ignore error in P6. */
			_vbi3_hamm24_inv_err[ii] = 0;
		} else if (0x20 == (i & 0x20)) {
			/* Double bit error. */
			_vbi3_hamm24_inv_err[ii] = 1 << 31;
		} else {
...
generate_hamm24_fwd_tables	(void)
{
	...
	/* Byte_0 = (  _vbi3_hamm24_fwd_0 [(d >> 0) & 0xFF]
	             ^ _vbi3_hamm24_fwd_1 [(d >> 8) & 0xFF]
	             ^ _vbi3_hamm24_fwd_2 [d >> 16];
	   D5_D11 = (d >> 4) & 0x7F;
	   D12_D18 = (d >> 11) & 0x7F;
	   P5 = 1 ^ parity (D12_D18);
	   Byte_1 = D5_D11 | (P5 << 7);
...
generate_hamm8_tables		(void)
{
	...
	memset (_vbi3_hamm8_inv, -1, sizeof (_vbi3_hamm8_inv));
...
do {									\
	...
	PRINT (const uint8_t, _vbi3_bit_reverse);
	PRINT (const uint8_t, _vbi3_hamm8_fwd);
	PRINT (const int8_t, _vbi3_hamm8_inv);
	PRINT (static const uint8_t, _vbi3_hamm24_fwd_0);
	PRINT (static const uint8_t, _vbi3_hamm24_fwd_1);
	PRINT (static const uint8_t, _vbi3_hamm24_fwd_2);
	...
	PRINT (static const uint8_t, _vbi3_hamm24_inv_d1_d4);

	printf ("static const int32_t\n_vbi3_hamm24_inv_err [64] = {");

	for (i = 0; i < 64; ++i) {
		printf ("%s0x%08x%s",
			0 == (i % 4) ? "\n\t" : "",
			_vbi3_hamm24_inv_err[i],
			i < 63 ? ", " : "");
	}
