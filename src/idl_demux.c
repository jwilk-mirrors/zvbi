...
/* $Id: idl_demux.c,v 1.3.2.6 2008-02-27 22:51:22 mschimek Exp $ */
...
static vbi3_bool
idl_a_demux_feed		(vbi3_idl_demux *	dx,
				 ...
				 int			ft)
{
	...
	flags = dx->flags | (ial & VBI3_IDL_DEPENDENT);
	dx->flags &= ~VBI3_IDL_DATA_LOST;
...
static vbi3_bool
idl_b_demux_feed		(vbi3_idl_demux *	dx,
				 const uint8_t		buffer[42],
				 int			ft)
{
	/* TODO */
...
static vbi3_bool
datavideo_demux_feed		(vbi3_idl_demux *	dx,
				 const uint8_t		buffer[42])
{
	/* TODO */
...
static vbi3_bool
audetel_demux_feed		(vbi3_idl_demux *	dx,
				 const uint8_t		buffer[42])
{
	/* TODO */
...
static vbi3_bool
lbra_demux_feed			(vbi3_idl_demux *	dx,
				 const uint8_t		buffer[42])
{
	/* TODO */
 ...
 * function given to vbi3_idl_a_demux_new() when new user data is available.
 *
 * @returns
				 ...
				 const uint8_t		buffer[42])
{
	...
	channel = vbi3_unham8 (buffer[0]);
	designation = vbi3_unham8 (buffer[1]);
	...
	case _VBI3_IDL_FORMAT_DATAVIDEO:
		return datavideo_demux_feed (dx, buffer);

	case _VBI3_IDL_FORMAT_AUDETEL: /* 6.7.2 */
		return audetel_demux_feed (dx, buffer);

	case _VBI3_IDL_FORMAT_LBRA: /* 6.7.3 */
				 ...
				 void *			user_data)
{
	...
	case _VBI3_IDL_FORMAT_DATAVIDEO:
	case _VBI3_IDL_FORMAT_B:
	case _VBI3_IDL_FORMAT_AUDETEL:
	case _VBI3_IDL_FORMAT_LBRA:
		/* TODO */
		break;
 ...
 *   vbi3_idl_a_demux_new(), can be @c NULL.
 *
 ...
 * @param callback Function to be called by vbi3_idl_demux_feed() when
 ...
 * freed with vbi3_idl_demux_delete() when done. @c NULL on failure
 * (out of memory).
 *
 * @since 0.2.14
 */
