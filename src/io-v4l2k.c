...
"$Id: io-v4l2k.c,v 1.2.2.19 2007-11-01 00:21:23 mschimek Exp $";
...
(IOCTL_ARG_TYPE_CHECK_ ## cmd (arg),					\
 _vbi3_log_ioctl (&v->capture.log, __FUNCTION__, snprint_ioctl_arg,	\
		  v->fd, cmd, (void *)(arg)))
...
typedef struct vbi_capture_v4l2 {
	...
	vbi3_bool		pal_start1_fix;
	vbi3_bool		saa7134_ntsc_fix;
	vbi3_bool		bttv_offset_fix;
	vbi3_bool		cx88_ntsc_fix;
	vbi3_bool		bttv_min_start_fix;
	vbi3_bool		bttv_ntsc_rate_fix;
...
static const vbi3_service_set VBI3_SERVICE_SET_SUPPORTED_BY_V4L2 =
	(VBI3_SLICED_TELETEXT_B |
	 VBI3_SLICED_VPS |
	 VBI3_SLICED_CAPTION_525 |
	 VBI3_SLICED_WSS_625);

static const unsigned int V4L2_MAX_FIELDS = 2;

static const unsigned int V4L2_MAX_SLICED_LINES =
	N_ELEMENTS (((struct v4l2_sliced_vbi_format *) 0)->service_lines[0]);

/* Helper functions. */

static vbi3_videostd_set
vbi3_videostd_set_from_v4l2_std_id
				(v4l2_std_id		set)
{
	if (0 == set) {
		return VBI3_VIDEOSTD_SET_UNKNOWN;
	} else {
		/* We check the correctness of this conversion
		   in test/test-io.cc. */
		return ((set & V4L2_STD_ALL) << 1);
	}
}

static vbi3_bool
vbi3_videostd_set_valid		(vbi3_videostd_set	set)
{
	/* Must be 525 xor 625. */

	if (set & VBI3_VIDEOSTD_SET_625_50) {
		return (0 == (set & VBI3_VIDEOSTD_SET_525_60));
	} else {
		return (0 != (set & VBI3_VIDEOSTD_SET_525_60));
	}
}

static vbi3_service_set
vbi3_service_from_v4l2_service	(unsigned int		id)
{
	switch (id) {
	case V4L2_SLICED_TELETEXT_B:
		return VBI3_SLICED_TELETEXT_B;

	case V4L2_SLICED_VPS:
		return VBI3_SLICED_VPS;

	case V4L2_SLICED_CAPTION_525:
		return VBI3_SLICED_CAPTION_525;

	case V4L2_SLICED_WSS_625:
		return VBI3_SLICED_WSS_625;
	}
...
static vbi3_service_set
vbi3_service_set_from_v4l2_service_set
				(unsigned int		set)
{
	vbi3_service_set services = 0;

	if (set & V4L2_SLICED_TELETEXT_B)
		services |= VBI3_SLICED_TELETEXT_B;

	if (set & V4L2_SLICED_VPS)
		services |= VBI3_SLICED_VPS;

	if (set & V4L2_SLICED_CAPTION_525)
		services |= VBI3_SLICED_CAPTION_525;

	if (set & V4L2_SLICED_WSS_625)
		services |= VBI3_SLICED_WSS_625;

	return services;
}

static unsigned int
vbi3_service_set_to_v4l2_service_set
				(vbi3_service_set	services)
{
	unsigned int set = 0;

	if (services & VBI3_SLICED_TELETEXT_B)
		set |= V4L2_SLICED_TELETEXT_B;

	if (services & VBI3_SLICED_VPS)
		set |= V4L2_SLICED_VPS;

	if (services & VBI3_SLICED_CAPTION_525)
		set |= V4L2_SLICED_CAPTION_525;

	if (services & VBI3_SLICED_WSS_625)
...
static void
vbi3_sliced_data_from_raw	(vbi3_capture_v4l2 *	v,
				 vbi3_capture_buffer **	sliced,
				 const vbi3_capture_buffer *raw)
{
	vbi3_capture_buffer *b;
	...
	b->size = n_lines * sizeof (vbi3_sliced);
	b->timestamp = raw->timestamp;
}


static void
v4l2_stream_stop(vbi3_capture_v4l2 *v)
{
	if (v->enqueue >= ENQUEUE_BUFS_QUEUED) {
		info (&v->capture.log, "Suspending stream.");
	...
	for (; v->num_raw_buffers > 0; v->num_raw_buffers--) {
		...
		b = &v->raw_buffer[v->num_raw_buffers - 1];
		_vbi3_log_munmap (&v->capture.log, __FUNCTION__,
				  b->data, b->size);
	}
...
static int
v4l2_stream_alloc(vbi3_capture_v4l2 *v, char ** errstr)
{
	...
	while (v->num_raw_buffers < vrbuf.count) {
		...
		p = _vbi3_log_mmap (&v->capture.log,
				    __FUNCTION__,
				    /* start: any */ NULL,
				    vbuf.length,
...
static vbi3_bool
restart_stream			(vbi3_capture_v4l2 *	v)
{
	unsigned int i;
...
v4l2_stream(vbi3_capture *vc, vbi3_capture_buffer **raw,
	    vbi3_capture_buffer **sliced, const struct timeval *timeout_orig)
{
	...
	if ((v->enqueue == ENQUEUE_SUSPENDED) || (v->services == 0)) {
		...
		error (&v->capture.log, "No services set or not committed.");
		errno = ESRCH;
		return -1;
	}
	...
	while (1)
	{
		...
		r = vbi3_capture_io_select(v->fd, &timeout);
		if (r <= 0) {
			if (r < 0) {
				error (&v->capture.log,
	...
	if (NULL != sliced) {
		vbi3_sliced_data_from_raw (v, sliced, b);
	}
...
static void
v4l2_stream_flush(vbi3_capture *vc)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
	struct timeval tv;
	...
	for (max_loop = 0; max_loop < v->num_raw_buffers; max_loop++) {
		...
		if (vbi3_capture_io_select(v->fd, &tv) <= 0)
			return;
...
static void
v4l2_read_stop(vbi3_capture_v4l2 *v)
{
	for (; v->num_raw_buffers > 0; v->num_raw_buffers--) {
...
static int
v4l2_suspend(vbi3_capture_v4l2 *v)
{
	int    fd;
	...
	if (v->streaming) {
		...
		if (v->read_active) {
			info (&v->capture.log, "Reopen device.");

			/* hack: cannot suspend read to allow S_FMT, need to close device */
			fd = _vbi3_log_open (&v->capture.log,
					     __FUNCTION__,
					     v->p_dev_name,
					     O_RDWR,
					     /* mode_t */ 0);
...
static int
v4l2_read_alloc(vbi3_capture_v4l2 *v, char ** errstr)
{
	assert(v->raw_buffer == NULL);
	...
	info (&v->capture.log, "Capture buffer allocated.");
...
static int
v4l2_read_frame(vbi3_capture_v4l2 *v, vbi3_capture_buffer *raw, struct timeval *timeout)
{
	int r;
...
v4l2_read(vbi3_capture *vc, vbi3_capture_buffer **raw,
	  vbi3_capture_buffer **sliced, const struct timeval *timeout)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
	...
	if (sliced) {
		vbi3_sliced_data_from_raw (v, sliced, *raw);
	}

	return 1;
}

static void v4l2_read_flush(vbi3_capture *vc)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
	struct timeval tv;
	int r;
	...
	r = vbi3_capture_io_select(v->fd, &tv);
	if (r <= 0)
		return;
...
static vbi3_bool
v4l2_get_videostd(vbi3_capture_v4l2 *v, char ** errstr)
{
	...
	if (-1 == xioctl (v, VIDIOC_G_STD, &std_id)) {
		asprintf (errstr,
	...
	v->sp.videostd_set = vbi3_videostd_set_from_v4l2_std_id (std_id);

	if (vbi3_videostd_set_valid (v->sp.videostd_set))
		return TRUE;

	/* Let's see if we can enumerate this standard. */
	...
	case 525:
		v->sp.videostd_set = VBI3_VIDEOSTD_SET_525_60;
		return TRUE;

	case 625:
		v->sp.videostd_set = VBI3_VIDEOSTD_SET_625_50;
		return TRUE;

	default:
		v->sp.videostd_set = VBI3_VIDEOSTD_SET_UNKNOWN;
		break;
	}

	/* FIXME this may be an USB camera or some other weird thing. */

	asprintf (errstr, _("Cannot determine the current "
...
static int
v4l2_get_scanning(vbi3_capture *vc)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
        vbi3_videostd_set old_scanning = v->sp.videostd_set;
        int new_scanning = -1;

        if ( v4l2_get_videostd(v, NULL) ) {
		new_scanning = 525;
		if (VBI3_VIDEOSTD_SET_625_50 & v->sp.videostd_set)
			new_scanning = 625;
        }
        v->sp.videostd_set = old_scanning;

        return new_scanning;
}

static void
print_vfmt			(vbi3_capture_v4l2 *	v,
				 const char *		s,
				 struct v4l2_format *	vfmt)
{
	if (0 == (v->capture.log.mask & VBI3_LOG_INFO))
		return;

	_vbi3_log_printf (v->capture.log.fn, v->capture.log.user_data,
			 VBI3_LOG_INFO, __FILE__, __FUNCTION__,
...
static unsigned int
v4l2_update_services(vbi3_capture *vc,
		     ...
		     char ** errstr)
{
	...
	info (&v->capture.log, "Querying current vbi parameters...");
	...
	if (strict >= 0) {
		struct v4l2_format vfmt_temp = vfmt;
		vbi3_sampling_par dec_temp;
		unsigned int f2_offset;
	        unsigned int sup_services;
		int r;

		info (&v->capture.log, "Attempt to set vbi capture parameters.");

		memset(&dec_temp, 0, sizeof(dec_temp));

		sup_services = _vbi3_sampling_par_from_services_log
			(&dec_temp, &max_rate,
			 v->sp.videostd_set,
//			 _vbi3_videostd_set_from_scanning (v->sp.scanning),
			 services | v->services, &v->capture.log);
		...
		if (0 == services) {
			...
			asprintf(errstr,
				 ...
				 v->p_dev_name, v->vcap.card, scanning);
			goto failure;
		}
		...
		f2_offset = (VBI3_VIDEOSTD_SET_625_50 & v->sp.videostd_set)
			? 312 : 263;
//		f2_offset = (625 == v->sp.scanning) ? 312 : 263;
		...
		if (v->pal_start1_fix
		    && 0 != (VBI3_VIDEOSTD_SET_625_50 & v->sp.videostd_set)) {
			vfmt.fmt.vbi.start[1] -= 1;
		}

		if (v->saa7134_ntsc_fix
		    && 0 != (VBI3_VIDEOSTD_SET_525_60 & v->sp.videostd_set)) {
...
#warning ok?
v->sp.samples_per_line	= vfmt.fmt.vbi.samples_per_line;
	...
	v->sp.sample_format	= VBI3_PIXFMT_YUV420;
	...
	** add fails later, to stay in sync with driver */
	...
	if (services & ~(VBI3_SLICED_VBI_525 | VBI3_SLICED_VBI_625)) {
		...
		if (v->sp.sampling_rate < max_rate * 3 / 2) {
		...
		info (&v->capture.log, "Nyquist check passed.");
		...
		if (strict > 0 && 0 != (services & vbi3_raw_decoder_services (&v->rd))) {
			...
			old_services = vbi3_raw_decoder_services (&v->rd);
			tmp_services = _vbi3_sampling_par_check_services_log
		...
		v->sliced_buffer.data =
			malloc((v->sp.count[0] + v->sp.count[1]) * sizeof(vbi3_sliced));
...
static VBI3_CAPTURE_FD_FLAGS
v4l2_get_fd_flags(vbi3_capture *vc)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
	VBI3_CAPTURE_FD_FLAGS result;

        result = VBI3_FD_IS_DEVICE | VBI3_FD_HAS_SELECT;
        if (v->streaming)
                result |= VBI3_FD_HAS_MMAP;

        return result;
}

static int
v4l2_get_fd(vbi3_capture *vc)
{
	vbi3_capture_v4l2 *v = PARENT(vc, vbi3_capture_v4l2, capture);
...
//	pthread_once (&vbi3_init_once, vbi3_init);
	...
	*errstr = NULL;
	...
	if (trace) {
		v->capture.log.fn = vbi3_log_on_stderr;
		v->capture.log.mask = VBI3_LOG_INFO * 2 - 1;

		vbi3_raw_decoder_set_log_fn (&v->rd,
					     /* mask */ VBI3_LOG_INFO * 2 - 1,
					     vbi3_log_on_stderr,
					     /* user_data */ NULL);
	}
	...
	if (-1 == fd) {
		v->fd = _vbi3_log_open (&v->capture.log,
					__FUNCTION__,
					v->p_dev_name,
					O_RDWR,
					/* mode_t */ 0);
	...
	if (v->vcap.capabilities & V4L2_CAP_STREAMING
	    && !vbi3_capture_force_read_mode) {
		info (&v->capture.log, "Using streaming interface.");
		...
		info (&v->capture.log, "Using read interface.");
 ...
 *   You can do so later with vbi3_capture_update_services(); note the
