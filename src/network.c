...
/* $Id: network.c,v 1.1.2.1 2004-02-13 02:15:27 mschimek Exp $ */
...
/* See about call signs http://www.fcc.gov/cgb/statid.html
   Note call sign implies country, e.g.
   US: K, N, W, AA-AL (TV stations K and W)
				 ...
				 vbi_nuid		nuid)
{
	...
	if (VBI_NUID_IS_TABLE (nuid)) {
		...
		l = ((nuid >> 56) & 31) + 1;
		l = MIN (l, sizeof (n->call) - 1);

		for (i = 0; i < l; ++i)
			n->call[i] = 0xFF & (nuid >> (i * 8));
		...
		case VBI_CNI_TYPE_PDC_B:
			n->cni_x26 = nuid & ~(((vbi_nuid) 0xFF) << 56);
			break;
				 ...
				 unsigned int		cni)
{
	...
	nuid = vbi_nuid_from_cni (from_type, cni);

	if (!VBI_NUID_IS_TABLE (nuid))
		return 0;

	if (!vbi_network_from_nuid (&n, nuid))
		return 0;
	...
	case VBI_CNI_TYPE_PDC_A:
		return cni_vps_to_pdc_a (n.cni_vps);
