 ...
 * vbi3_decode_dvb_pdc_descriptor() function is available to convert a PDC
 * descriptor to a VPS PIL.
 */

/* XXX Preliminary. */
enum {
	VBI3_ERR_BUFFER_OVERFLOW		= 0x7080600,
	VBI3_ERR_SLICED_BUFFER_OVERFLOW	= 0x7080601,
	VBI3_ERR_RAW_BUFFER_OVERFLOW,
	VBI3_ERR_SYNC_LOST		= 0x7080700,
	VBI3_ERR_SCRAMBLED		= 0x7080800,
	VBI3_ERR_STREAM_SYNTAX		= 0x7080900,
	VBI3_ERR_DU_OVERFLOW		= 0x7080901,
	VBI3_ERR_DU_LENGTH,
	VBI3_ERR_DU_LINE_NUMBER,
	VBI3_ERR_DU_RAW_SEGMENT_POSITION,
	VBI3_ERR_DU_RAW_SEGMENT_LOST,
	VBI3_ERR_DU_RAW_DATA_INCOMPLETE,
	VBI3_ERR_CALLBACK		= 0x7080a00,
};
...
struct frame {
	 ...
	 * field respectively.
	 * XXX to be replaced by struct vbi3_sampling_par.
	 */
...
typedef vbi3_bool
demux_packet_fn			(vbi3_dvb_demux *	dx,
				 const uint8_t **	src,
				 unsigned int *		src_left);
...
struct _vbi3_dvb_demux {
	...
	/** Output buffer for vbi3_dvb_demux_demux(). */
 ...
 * @param spp A pointer to a vbi3_sliced structure will be stored here.
 ...
 * unit, allocating a vbi3_sliced (and if requested raw VBI data)
 ...
 *   allocated.
 * - @c VBI3_ERR_SLICED_BUFFER_OVERFLOW
				 ...
				 enum systems		system)
{
	...
	if (unlikely (f->sp >= f->sliced_end)) {
		...
		return VBI3_ERR_SLICED_BUFFER_OVERFLOW;
	}
	...
	if (0 != frame_line) {
		if (frame_line <= f->last_frame_line) {
			if (f->n_data_units_extracted_from_packet > 0) {
				...
				return VBI3_ERR_DU_LINE_NUMBER;
			}
				 ...
				 enum systems		system)
{
	...
	if (unlikely (0 == n_pixels || first_pixel_position >= 720)) {
		...
		return VBI3_ERR_DU_RAW_SEGMENT_POSITION;
	}
	...
	if ((int8_t) p[2] < 0) {
		...
		if (unlikely (f->raw_offset > 0)) {
			...
			return VBI3_ERR_DU_RAW_DATA_INCOMPLETE;
		}
		...
		s->id = (SYSTEM_525 == system) ?
			VBI3_SLICED_VBI_525 : VBI3_SLICED_VBI_625;
	} else {
		unsigned int field;
		unsigned int field_line;
		unsigned int frame_line;
		...
		if (unlikely (0 == f->raw_offset)) {
			...
			return VBI3_ERR_DU_RAW_SEGMENT_LOST;
		}
				 ...
				 const vbi3_sliced *	s)
{
	...
	for (i = 0; i < 42; ++i)
		buffer[i] = _vbi3_to_ascii (s->data[i]);
	buffer[i] = 0;
 ...
 *
 * Converts the data units in a VBI PES packet to vbi3_sliced data
 ...
 * the function again, perhaps after calling _vbi3_dvb_skip_data_unit().
 *
				 ...
				 unsigned int *		src_left)
{
	...
	while (p < p_end_m2) {
		...
		case DATA_UNIT_EBU_TELETEXT_SUBTITLE:
			...
			/* FIXME */
			if (unlikely (0xE4 != p[3])) { /* vbi3_rev8 (0x27) */
			        warning (&f->log,
 ...
 * streams consumed by vbi3_dvb_demux_cor() or vbi3_dvb_demux_feed().
 *
 * @returns
...
static vbi3_bool
decode_timestamp		(vbi3_dvb_demux *	dx,
				 int64_t *		pts,
				 unsigned int		mark,
				 const uint8_t *	p)
{
	...
	if (dx->frame.log.mask & VBI3_LOG_DEBUG) {
		int64_t old_pts;
		int64_t new_pts;
 ...
 *     see vbi3_dvb_demux_cor() coroutine) or the callback function
 ...
 *     _vbi3_dvb_skip_data_unit(), then call again as above.
 * - @c Error in the VBI3_ERR_STREAM_SYNTAX range
 ...
 * the error occurred, set dx->new_frame to TRUE. VBI3_ERR_CALLBACK
 * implies that @a src points to a new PES packet and dx->new_frame
 * will be already TRUE.
 */
static int
demux_pes_packet_frame		(vbi3_dvb_demux *	dx,
				 const uint8_t **	src,
				 unsigned int *		src_left)
{
	for (;;) {
 ...
 */
static int
demux_pes_packet		(vbi3_dvb_demux *	dx,
				 const uint8_t **	src,
				 unsigned int *		src_left)
{
	const uint8_t *s;
 ...
 * - @c VBI3_ERR_SCRAMBLED
 *   - The payload is scrambled.
 * - @c VBI3_ERR_STREAM_SYNTAX
 ...
 */
static int
demux_ts_packet			(vbi3_dvb_demux *	dx,
				 const uint8_t **	src,
				 unsigned int *		src_left)
{
	const uint8_t *s;
 ...
 * Packetized Elementary Stream (PES), filters
 * out PRIVATE_STREAM_1 PES packets, filters out valid VBI data units,
 * converts them to vbi3_sliced format and calls the vbi3_dvb_demux_cb
 * function given to vbi3_dvb_pes_demux_new() when a new frame is complete.
 *
 * @returns
 ...
 * @param dx DVB demultiplexer context allocated with vbi3_dvb_pes_demux_new().
 *
 * Resets the DVB demux to the initial state as after vbi3_dvb_pes_demux_new(),
 ...
 *   vbi3_log_on_stderr(). Can be @c NULL to disable logging.
 ...
 * vbi3_set_log_fn()), or enable logging only in the DVB
 * demultiplexer @a dx.
 *
 * @note
 ...
 *   vbi3_dvb_pes_demux_new(), can be @c NULL.
 *
 ...
 * freed with vbi3_dvb_demux_delete() when done. @c NULL on failure
 * (out of memory).
 *
 * @since 0.2.10
 */
...
_vbi3_dvb_demux_pes_new		(vbi3_dvb_demux_cb *	callback,
				 void *			user_data);
...
_vbi3_dvb_demux_cor		(vbi3_dvb_demux *	dx,
				 ...
				 unsigned int *		buffer_left)
{
	return vbi3_dvb_demux_cor (dx, sliced, sliced_lines,
...
_vbi3_dvb_demux_pes_new		(vbi3_dvb_demux_cb *	callback,
				 void *			user_data)
{
	return vbi3_dvb_pes_demux_new (callback, user_data);
}
