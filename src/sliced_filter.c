...
/* $Id: sliced_filter.c,v 1.4.2.3 2008-02-27 22:49:48 mschimek Exp $ */
...
/* 0 ... (VBI3_ANY_SUBNO = 0x3F7F) - 1. */
#define MAX_SUBNO 0x3F7E

struct _vbi3_sliced_filter {
	vbi3_page_table *	keep_ttx_pages;

	vbi3_bool		keep_ttx_system_pages;
	...
	vbi3_sliced_filter_cb *	callback;
	void *			user_data;
};

static void
set_errstr			(vbi3_sliced_filter *	sf,
				 const char *		templ,
				 ...)
{
	va_list ap;
...
static void
no_mem_error			(vbi3_sliced_filter *	sf)
{
	vbi3_free (sf->errstr);
...
vbi3_bool
vbi3_sliced_filter_drop_cc_channel
...
vbi3_bool
vbi3_sliced_filter_keep_cc_channel
				(vbi3_sliced_filter *	sf,
				 vbi3_pgno		channel)
{
}
...
valid_ttx_page			(vbi3_pgno		pgno)
{
	return ((unsigned int) pgno - 0x100 < 0x800);
}

static vbi3_bool
valid_ttx_subpage_range		(vbi3_sliced_filter *	sf,
				 vbi3_pgno		pgno,
				 ...
				 vbi3_subno		last_subno)
{
	...
	if (sf->keep_services & VBI3_SLICED_TELETEXT_B_625) {
		vbi3_page_table_add_all_pages (sf->keep_ttx_pages);
		sf->keep_services &= ~VBI3_SLICED_TELETEXT_B_625;
	}

	return vbi3_page_table_remove_subpages (sf->keep_ttx_pages,
						pgno,
						first_subno,
						last_subno);
}
				 ...
				 vbi3_subno		last_subno)
{
	...
	return vbi3_page_table_add_subpages (sf->keep_ttx_pages,
					     pgno, first_subno, last_subno);
}

static vbi3_bool
valid_ttx_page_range		(vbi3_sliced_filter *	sf,
				 ...
				 vbi3_pgno		last_pgno)
{
	...
	return vbi3_page_table_remove_pages (sf->keep_ttx_pages,
					     first_pgno, last_pgno);
}
				 ...
				 vbi3_pgno		last_pgno)
{
	...
	return vbi3_page_table_add_pages (sf->keep_ttx_pages,
					  first_pgno, last_pgno);
}
				 ...
				 vbi3_service_set	services)
{
	...
	if (services & VBI3_SLICED_TELETEXT_B_625)
		vbi3_page_table_remove_all_pages (sf->keep_ttx_pages);
...
static vbi3_bool
decode_teletext_packet_0	(vbi3_sliced_filter *	sf,
				 ...
				 unsigned int		magazine)
{
	...
	if (!vbi3_is_bcd (pgno)) {
		...
		if (vbi3_page_table_contains_subpage (sf->keep_ttx_pages,
						      pgno, subno))
			goto match;
	}
...
static vbi3_bool
decode_teletext			(vbi3_sliced_filter *	sf,
				 vbi3_bool *		keep,
				 const uint8_t		buffer[42],
				 unsigned int		line)
{
 ...
 * vbi3_sliced_filter_new() with a pointer to the filtered lines.
 *
 * @returns
				 ...
				 unsigned int *		n_lines)
{
	...
	if (unlikely (sf->output_max_lines < *n_lines)) {
		...
		n = MIN (*n_lines, 50U);
		s = vbi3_realloc (sf->output_buffer,
...
vbi3_sliced_filter_delete	(vbi3_sliced_filter *	sf)
{
	...
	vbi3_page_table_delete (sf->keep_ttx_pages);

	vbi3_free (sf->output_buffer);
	vbi3_free (sf->errstr);
				 ...
				 void *			user_data)
{
	...
	sf->keep_ttx_pages = vbi3_page_table_new ();
	if (NULL == sf->keep_ttx_pages) {
		vbi3_free (sf);
		return NULL;
	}

	vbi3_sliced_filter_reset (sf);
