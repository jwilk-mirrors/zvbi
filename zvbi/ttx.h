...
#include "page.h"		/* vbi_color, vbi_rgba */
#include "ttx_charset.h"	/* vbi_ttx_charset_code */
...
_vbi_ttx_page_function_valid	(enum ttx_page_function	function)
{
	return ((unsigned int) function
		<= (unsigned int) PAGE_FUNCTION_TRIGGER);
}
...
_vbi_inline vbi_bool
_vbi_ttx_page_coding_valid	(enum ttx_page_coding	coding)
{
...
struct ttx_extension {
	...
	/**
	 * The "default screen color" in EN 300 706 parlance, the
	 * color of the border around the text.
	 */
	unsigned int			def_border_color;
...
/**
 * @internal
 * Defaults for all pages in a magazine.
 */
struct ttx_magazine {
	/** Default extension. */
