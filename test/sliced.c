...
vbi3_bool			option_quiet;
unsigned int			option_log_mask;

static vbi3_bool		have_dev_name;
				 ...
				 int64_t		stream_time)
{
	...
	success = vbi3_dvb_mux_feed (st->mx,
				     sliced, n_lines,
	...
	if (!success) {
		...
		error_exit (_("Maximum PES packet size %u bytes "
			    ...
			    vbi3_dvb_mux_get_max_pes_packet_size (st->mx));
	}
...
service_map [] = {
	{ "TELETEXT_B",		VBI3_SLICED_TELETEXT_B,			42 },
	{ "CAPTION_625",	VBI3_SLICED_CAPTION_625,		2 },
	{ "VPS",		VBI3_SLICED_VPS | VBI3_SLICED_VPS_F2,	13 },
	{ "WSS_625",		VBI3_SLICED_WSS_625,			2 },
	{ "WSS_CPR1204",	VBI3_SLICED_WSS_CPR1204,		3 },
	{ NULL, 0, 0 },
	{ NULL, 0, 0 },
	{ "CAPTION_525",	VBI3_SLICED_CAPTION_525,		2 },
};
				 ...
				 unsigned int		n_lines)
{
	...
	for (; sliced < sliced_end; ++sliced) {
		...
		if (VBI3_SLICED_VBI_525 == sliced->id
		    || VBI3_SLICED_VBI_625 == sliced->id)
			continue;
				 ...
				 const vbi3_sampling_par *sp)		 
{
	...
	format = vbi3_pixfmt_name (sp->sp_sample_format);
				 ...
				 unsigned int		data_identifier)
{
	...
	if (!vbi3_dvb_mux_set_data_identifier (st->mx, data_identifier)) {
				 ...
				 unsigned int		max)
{
	...
	if (!vbi3_dvb_mux_set_pes_packet_size (st->mx, min, max))
		no_mem_exit ();
}
				 ...
				 unsigned int		system)
{
	...
	case FILE_FORMAT_DVB_PES:
		...
		st->mx = vbi3_dvb_pes_mux_new (pes_ts_cb,
					       /* user_data */ st);
		if (NULL == st->mx)
			no_mem_exit ();
	...
	case FILE_FORMAT_DVB_TS:
		...
		st->mx = vbi3_dvb_ts_mux_new (ts_pid,
					      pes_ts_cb,
					      /* user_data */ st);
		if (NULL == st->mx)
			no_mem_exit ();
...
read_loop_pes_ts		(struct stream *	st)
{
	for (;;) {
		...
		n_lines = vbi3_dvb_demux_cor (st->dx,
					     st->sliced,
...
#define r32(v) v = p[0] | (p[1] << 8) | (p[2] << 16) | (p[3] << 24); p += 4
	r16 (system);
	sp->sp_sample_format = VBI3_PIXFMT_Y8;
...
#  error VBI_VERSION_MINOR == ?
	...
	if (st->debug) {
		if (NULL == st->rd) {
			...
			st->rd = vbi3_raw_decoder_new (sp);
			if (NULL == st->rd)
				return p;
			vbi3_raw_decoder_add_services (st->rd, -1, 1);
			success = vbi3_raw_decoder_debug (st->rd, TRUE);
			assert (success);
		}

		st->sliced2_lines =
			vbi3_raw_decoder_decode (st->rd,
						 st->sliced2,
				 ...
				 stream_callback_fn *	callback)
{
	...
	case FILE_FORMAT_DVB_PES:
		...
		st->dx = vbi3_dvb_pes_demux_new (/* callback */ NULL,
						/* user_data */ NULL);
		if (NULL == st->dx)
			no_mem_exit ();
	...
	case FILE_FORMAT_DVB_TS:
		...
		st->dx = _vbi3_dvb_ts_demux_new (/* callback */ NULL,
						 /* user_data */ NULL,
						 ts_pid);
		if (NULL == st->dx)
			no_mem_exit ();
...
capture_loop			(struct stream *	st)
{
	...
	for (;;) {
		...
		if (st->interfaces & INTERFACE_DVB) {
			stream_time = vbi3_capture_dvb_last_pts (st->cap);
		} else {
				 ...
				 unsigned int		flags)
{
	...
	if (st->interfaces & INTERFACE_SIM) {
		_vbi3_capture_sim_set_flags (st->cap, flags);

		if (flags & _VBI3_RAW_NOISE_2) {
			vbi3_capture_sim_add_noise (st->cap,
						    /* min_freq */ 0,
						    /* max_freq */ 5000000,
						    /* amplitude */ 25);
		} else {
			vbi3_capture_sim_add_noise (st->cap, 0, 0, 0);
		}
	}
}
				 ...
				 vbi3_bool		enable)
{
	...
	if (st->interfaces & INTERFACE_SIM)
		vbi3_capture_sim_decode_raw (st->cap, enable);
}
				 ...
				 vbi3_bool		append)
{
	...
	return FALSE;
#elif 3 == VBI_VERSION_MINOR
	return vbi3_capture_sim_load_caption (st->cap, stream, append);
#else
#  error VBI_VERSION_MINOR == ?
#endif
}
...
#elif 3 == VBI_VERSION_MINOR
	if (NULL != st->cap) {
		return vbi3_capture_sampling_point
			(st->cap, point, row, nth_bit);
	} else if (NULL != st->rd) {
		if (!st->raw_valid)
			return FALSE;
		return vbi3_raw_decoder_sampling_point
				 ...
				 vbi3_bool		enable)
{
	...
	if (NULL != st->cap) {
		return vbi3_capture_debug (st->cap, enable);
	} else if (NULL != st->rd) {
		return vbi3_raw_decoder_debug (st->rd, enable);
	} else {
		return FALSE;
	}
#else
				 ...
				 stream_callback_fn *	callback)
{
	...
	trace = (0 != (option_log_mask & VBI3_LOG_INFO));

	if (interfaces & INTERFACE_SIM) {
		st->cap = vbi3_capture_sim_new (system,
						&services,
						sim_interlaced,
						sim_synchronous);
		if (NULL == st->cap)
...
#  error VBI_VERSION_MINOR == ?
	...
	if (interfaces & INTERFACE_V4L2) {
		...
		st->cap = vbi3_capture_v4l2_new (dev_name,
						 n_buffers,
						 &services,
						 strict,
						 &errstr,
	...
	if (interfaces & INTERFACE_V4L) {
		...
		st->cap = vbi3_capture_v4l_new (dev_name,
						system,
						&services,
						strict,
						&errstr,
	...
	if (interfaces & INTERFACE_BKTR) {
		...
		st->cap = vbi3_capture_bktr_new (dev_name,
						 system,
						 &services,
						 strict,
						 &errstr,
	...
	if (interfaces & (INTERFACE_SIM |
		...
		st->sp = *vbi3_capture_parameters (st->cap);
...
parse_option_verbose		(void)
{
	...
	vbi3_set_log_fn (option_log_mask,
			 vbi3_log_on_stderr,
			 /* user_data */ NULL);
}
				 ...
				 char **		argv)
{
	...
	option_log_mask = VBI3_LOG_NOTICE * 2 - 1;
