/*
 *  libzvbi - vbi3_dvb_mux unit test
 *
...
// XXX Later.
enum {
	VBI3_ERR_BUFFER_OVERFLOW = 0,
	VBI3_ERR_AMBIGUOUS_VIDEOSTD = 0,
	VBI3_ERR_LINE_NUMBER = 0,
	VBI3_ERR_LINE_ORDER = 0,
	VBI3_ERR_INVALID_SERVICE = 0,
	VBI3_ERR_SAMPLE_NUMBER = 0,
	VBI3_ERR_NO_RAW_DATA = 0,
	VBI3_ERR_SAMPLING_PAR = 0,
};
...
static const vbi3_videostd_set ANY_VIDEOSTD = 0x12345;

static const vbi3_service_set ALL_SERVICES = -1;
...
static vbi3_bool
is_good_service			(vbi3_service_set	service)
{
	unsigned int i;
...
all_services [] = {
	...
	VBI3_SLICED_CAPTION_625 | VBI3_SLICED_WSS_625,
	VBI3_SLICED_CAPTION_625,
	VBI3_SLICED_CAPTION_625_F1,
	VBI3_SLICED_CAPTION_625_F2,
	VBI3_SLICED_TELETEXT_A,
	VBI3_SLICED_TELETEXT_BD_525,
	VBI3_SLICED_TELETEXT_B_525,
	VBI3_SLICED_TELETEXT_B_625 | VBI3_SLICED_VPS,
	VBI3_SLICED_TELETEXT_B_625,
	VBI3_SLICED_TELETEXT_B_L10_625,
	VBI3_SLICED_TELETEXT_B_L25_625,
	...
	VBI3_SLICED_VBI_525,
	VBI3_SLICED_VBI_625,
	VBI3_SLICED_VPS | VBI3_SLICED_CAPTION_625,
...
alloc_sliced			(unsigned int		n_lines)
{
	...
	sliced = (vbi3_sliced *) xmalloc (n_lines * sizeof (vbi3_sliced));

	/* Must initialize the .data[] arrays for valgrind. We fill
	   with 0xFF because zeros have special meaning. */
	memset (sliced, -1, n_lines * sizeof (vbi3_sliced));

	return sliced;
}

static uint8_t *
alloc_raw_frame			(const vbi3_sampling_par *sp)
{
				 ...
				 vbi3_bool		fixed_length)
{
	/* Verify the value of reserved bits and stuffing bytes
	   which are ignored by the vbi3_dvb_demux. */
	...
	*n_stuffing_dus = 0;
	...
	while (n_bytes >= 2) {
		...
		case 0xB4: /* DATA_UNIT_ZVBI3_WSS_CPR1204 */
			// Should not appear here.
			assert (0);

		case 0xB5: /* DATA_UNIT_ZVBI3_CLOSED_CAPTION_525 */
			// Should not appear here.
			assert (0);

		case 0xB6: /* DATA_UNIT_ZVBI3_MONOCHROME_SAMPLES_525 */
			// Should not appear here.
			assert (0);
				 ...
				 unsigned int		n_pixels_total)
{
	...
	*n_stuffing_dus = 0;
	...
	if (0 != (videostd_set & VBI3_VIDEOSTD_SET_525_60)) {
		assert (0 == (videostd_set & VBI3_VIDEOSTD_SET_625_50));
		f2_start = 263;
	} else if (0 != (videostd_set & VBI3_VIDEOSTD_SET_625_50)) {
		f2_start = 313;
	} else {
		assert (0);
	}
...
static void
assert_same_sliced		(const vbi3_sliced *	sliced_in,
				 unsigned int		n_lines_in,
				 const vbi3_sliced *	sliced_out,
				 ...
				 vbi3_service_set	service_mask)
{
	...
	for (i_in = 0; i_in < n_lines_in; ++i_in) {
		vbi3_service_set id_in;
		vbi3_service_set id_out;
		...
		payload_bits = vbi3_sliced_payload_bits (id_in);
				 ...
				 vbi3_service_set	service_mask)
{
	...
	success = _vbi3_dvb_demultiplex_sliced (sliced_out,
						&n_lines_out,
						max_lines_out,
						&p,
						&p_left);
				 ...
				 int64_t		pts_in)
{
	...
	n_lines_out = vbi3_dvb_demux_cor (dx,
					  sliced_out,
					  max_lines_out,
					  &pts_out,
					  &p,
...
/*
	Test vbi3_dvb_multiplex_sliced().
 */

static void
assert_multiplex_sliced		(uint8_t * const	p1,
				 const unsigned int	p1_size,
				 const vbi3_sliced * const s1,
				 const unsigned int	s1_lines,
				 const vbi3_service_set	service_mask,
				 unsigned int		data_identifier,
				 vbi3_bool		stuffing,
				 vbi3_bool		exp_success,
				 int			exp_errno,
				 ...
				 unsigned int		exp_consumed_lines)
{
	...
	if (ANY_DATA_IDENTIFIER == data_identifier) {
		...
		if (exp_success) {
			if (0 == p1_size % 46) {
				...
				exp_success = FALSE;
				exp_errno = VBI3_ERR_BUFFER_OVERFLOW;
				exp_out_lines = 0;
				exp_out_data_size = 0;
	...
	success = vbi3_dvb_multiplex_sliced (&p, &p_left,
					     &s, &s_left,
					     service_mask,
					     data_identifier,
					     stuffing);
...
test_multiplex_sliced_stuffing	(void)
{
	...
	sliced[0].id = VBI3_SLICED_TELETEXT_B_625;
	sliced[0].line = 7;
...
static void
test_ms_line			(vbi3_service_set	service,
				 unsigned int		line,
				 vbi3_bool		correct)
{
	vbi3_sliced *sliced;
...
static vbi3_bool
is_correct_line			(vbi3_service_set	service,
				 unsigned int		field,
				 unsigned int		line_offset)
{
	switch (service) {
				...
				(vbi3_service_set	service)
{
	...
	for (i = 0; i < 6; ++i) {
		sliced[i].id = VBI3_SLICED_TELETEXT_B_625;
		sliced[i].line = i + 7;
	}

	if (service & VBI3_SLICED_VPS)
		line = 16;
	else if (service & VBI3_SLICED_CAPTION_625)
		line = 21;
	else if (service & VBI3_SLICED_WSS_625)
		line = 23;
	else
		line = 13;
	...
	sliced[7].id = VBI3_SLICED_TELETEXT_B_625;
	sliced[7].line = 320;

	if (is_good_service (service)) {
		unsigned int exp_out_lines;

		if (VBI3_SLICED_NONE == service)
	...
	if (-1u == service
	    || (VBI3_SLICED_TELETEXT_B_625
		== (VBI3_SLICED_TELETEXT_B_625 & service))) {
		assert_multiplex_sliced (buffer, buffer_size,
					 sliced, n_lines,
				 ...
				 vbi3_bool		stuffing)
{
	...
	for (i = 7; i < 16; ++i) {
		sliced[n_lines].id = VBI3_SLICED_TELETEXT_B_625;
	...
	sliced[n_lines].id = VBI3_SLICED_VPS;
	...
	sliced[n_lines].id = VBI3_SLICED_CAPTION_625;
	...
	sliced[n_lines].id = VBI3_SLICED_WSS_625;
				 ...
				 const int		exp_errno)
{
	...
	success = vbi3_dvb_multiplex_raw (&p, &p_left,
					  &r, &r_left,
					  data_identifier,
					  videostd_set,
					  line,
...
test_mr_line			(unsigned int		line,
				 vbi3_bool		exp_success_525,
				 vbi3_bool		exp_success_625)
{
	uint8_t *buffer;
	uint8_t *raw;
	unsigned int buffer_size;
...
test_multiplex_raw_videostd_checks (void)
{
	...
	assert_multiplex_raw (buffer, buffer_size,
			      ...
			      /* raw_size */ 720,
			      ANY_DATA_IDENTIFIER,
			      (VBI3_VIDEOSTD_SET (VBI3_VIDEOSTD_PAL_B) |
			       VBI3_VIDEOSTD_SET (VBI3_VIDEOSTD_NTSC_M)),
			      /* line */ 10,
			      /* first_pixel_position */ 0,
			      /* n_pixels_total */ 720,
			      ANY_STUFFING,
...
#else
	/* scanning */		625,
	/* sampling_format */	VBI3_PIXFMT_YUV420,
	/* sampling_rate */	13500000,
	/* bytes_per_line */	720,
	/* offset */		132,
	/* start */		{ 7, 320 },
...
public:
	...
	~DVBMux () {
		vbi3_dvb_mux_delete (_mx);
	};

	operator vbi3_dvb_mux* () const {
		return _mx;
	};

	unsigned int get_min_pes_packet_size () {
		assert (NULL != _mx);
		return vbi3_dvb_mux_get_min_pes_packet_size (_mx);
	};

	unsigned int get_max_pes_packet_size () {
		assert (NULL != _mx);
		return vbi3_dvb_mux_get_max_pes_packet_size (_mx);
	};

	bool set_pes_packet_size (unsigned int min_size,
				  unsigned int max_size) {
		assert (NULL != _mx);
		return vbi3_dvb_mux_set_pes_packet_size (_mx,
							 min_size,
							 max_size);
	};
	...
	unsigned int get_data_identifier () {
		assert (NULL != _mx);
		return vbi3_dvb_mux_get_data_identifier (_mx);
	};

	bool set_data_identifier (unsigned int di) {
		assert (NULL != _mx);
		return vbi3_dvb_mux_set_data_identifier (_mx, di);
	};
};

class DVBPESMux : public DVBMux {
public:
	DVBPESMux (vbi3_dvb_mux_cb *		callback = NULL,
		   void *			user_data = NULL) {
		_mx = vbi3_dvb_pes_mux_new (callback, user_data);
		assert (NULL != _mx);
	};
};

static void
alloc_init_sliced		(vbi3_sliced **		sliced_p,
				 unsigned int *		n_lines_p)
{
	...
	for (field = 0; field < 2; ++field) {
		...
		if (0 == field)
			sliced[n_lines].id = VBI3_SLICED_VBI_625;
		else
			sliced[n_lines].id = VBI3_SLICED_TELETEXT_B_625;
...
class DVBPESMuxTest : public DVBPESMux {
	...
	uint8_t *		_raw;
	const vbi3_sampling_par *_sp;
	bool			_have_raw;
	bool			_free_raw;

	vbi3_service_set	_service_mask;
	...
	void cmp_ts_feed (const vbi3_bool exp_success,
			  const int exp_errno,
			  const unsigned int exp_consumed_lines);

	void cmp_ts_cor (const vbi3_bool exp_success,
			 const int exp_errno,
			 const unsigned int exp_consumed_lines);

	void cmp_pes_feed (const vbi3_bool exp_success,
			   const int exp_errno,
			   const unsigned int exp_consumed_lines);

	void copy_props (vbi3_dvb_mux *mx) {
		vbi3_bool success;

		assert (NULL != mx);
	
		success = vbi3_dvb_mux_set_data_identifier
			(mx, get_data_identifier ());
		assert (TRUE == success);

		success = vbi3_dvb_mux_set_pes_packet_size
...
public:
	...
	void set_sliced (vbi3_sliced* sliced,
			 unsigned int n_lines) {
		if (_free_sliced)
			free (_sliced);
	...
	void set_sampling_par (const vbi3_sampling_par *sp) {
		if (_free_raw)
			free (_raw);
	...
	void set_service_mask (vbi3_service_set mask) {
		_service_mask = mask;
	};
	...
	void test (const vbi3_bool exp_success,
		   const int exp_errno,
		   const unsigned int exp_consumed_lines);

	void test_line (const vbi3_sampling_par *sp,
			const vbi3_service_set service,
			const unsigned int line,
			const vbi3_bool exp_success);
...
static vbi3_bool
dvb_mux_pes_cb			(vbi3_dvb_mux *		mx,
				 void *			user_data,
...
void
DVBPESMuxTest::test		(const vbi3_bool 	exp_success,
				 const int		exp_errno,
				 const unsigned int	exp_consumed_lines)
{
	uint8_t *rand_buffer;
	...
	const vbi3_sliced *s = _sliced;
	unsigned int s_left = _sliced_lines;

	vbi3_bool success = vbi3_dvb_mux_cor (*this,
					      &p, &p_left,
					      &s, &s_left,
					      _service_mask,
					      _raw, _sp,
	...
	if (NULL != _buffer && _buffer_size > 0) {
		...
		mx = vbi3_dvb_pes_mux_new (/* callback */ dvb_mux_pes_cb,
					   /* user_data */ this);
		assert (NULL != mx);
...

	{
		...
		mx = vbi3_dvb_ts_mux_new (/* pid */ 0x1234,
					  /* callback */ NULL,
					  /* user_data */ NULL);
		assert (NULL != mx);
...
void
DVBPESMuxTest::test_line	(const vbi3_sampling_par *sp,
				 const vbi3_service_set	service,
				 const unsigned int	line,
				 const vbi3_bool	exp_success)
{
...
test_dvb_mux_cor_service_mask (void)
{
	...
	mx.set_service_mask (VBI3_SLICED_VPS |
			     VBI3_SLICED_WSS_625);
	mx.test_pass ();
				 ...
				 unsigned int		offset)
{
	...
	if (offset < 132
		...
		mx.set_sliced (/* sliced */ (vbi3_sliced *) -1,
			       /* sliced_lines */ 17);
		mx.set_raw ((uint8_t *) -1, &sp);
		mx.test_fail (VBI3_ERR_SAMPLING_PAR,
			      /* exp_consumed_lines */ 0);
...
test_dvb_mux_cor_sampling_parameter_checks (void)
{
	...
	// FIXME: Test vbi3_valid_sampling_par_log().
	...
	sp = good_par_625;
	sp.videostd_set = VBI3_VIDEOSTD_SET_525_60;
	mx.set_sampling_par (&sp);
	mx.test_fail (VBI3_ERR_SAMPLING_PAR,
		      /* exp_consumed_lines */ 0);

	sp = good_par_625;
	sp.videostd_set = (VBI3_VIDEOSTD_SET (VBI3_VIDEOSTD_PAL_B) |
			   VBI3_VIDEOSTD_SET (VBI3_VIDEOSTD_NTSC_M));
	mx.set_sampling_par (&sp);
	mx.test_fail (VBI3_ERR_SAMPLING_PAR,
		      /* exp_consumed_lines */ 0);
	...
	sp = good_par_625;
	sp.sp_sample_format = VBI3_PIXFMT_YUYV;
	mx.set_sampling_par (&sp);
	mx.test_fail (VBI3_ERR_SAMPLING_PAR,
		      /* exp_consumed_lines */ 0);
...
test_dvb_mux_cor_null_raw_or_sp_checks (void)
{
	...
	mx.test_fail (VBI3_ERR_NO_RAW_DATA,
...
test_dvb_mux_cor_sp_line_number_checks (void)
{
	...
	sp = good_par_625;
	sp.count[0] = 0;
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 7,
		      /* exp_success */ FALSE);

	sp = good_par_625;
	sp.count[1] = 0;
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 320,
		      /* exp_success */ FALSE);
	...
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 7,
		      /* exp_success */ FALSE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 8,
		      /* exp_success */ TRUE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 22,
		      /* exp_success */ TRUE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 23,
		      /* exp_success */ FALSE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 313 + 7,
		      /* exp_success */ FALSE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 313 + 8,
		      /* exp_success */ TRUE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 313 + 22,
		      /* exp_success */ TRUE);
	mx.test_line (&sp, VBI3_SLICED_VBI_625, 313 + 23,
		      /* exp_success */ FALSE);
}
...
test_dvb_mux_cor_line_number_checks (void)
{
	...
	for (i = 0; i <= 31; ++i) {
		...
		for (j = 0; j < N_ELEMENTS (good_services); ++j) {
			vbi3_service_set service = good_services[j];
		...
		vbi3_service_set service = VBI3_SLICED_VBI_625;
...
test_dvb_mux_cor_service_checks (void)
{
	...
	for (i = 0; i < N_ELEMENTS (all_services); ++i) {
		...
		if (VBI3_SLICED_VBI_625 == service
		    || is_good_service (service)) {
			mx.test_pass ();
		} else {
			mx.test_fail (VBI3_ERR_INVALID_SERVICE,
				      /* exp_consumed_lines */ 6);
		}
	}
...
test_dvb_mux_cor_line_order_checks (void)
{
	...
	assert (VBI3_SLICED_TELETEXT_B_625 == sliced[1].id);
	assert (8 == sliced[1].line);
	assert (VBI3_SLICED_TELETEXT_B_625 == sliced[2].id);
	assert (9 == sliced[2].line);
	assert (VBI3_SLICED_TELETEXT_B_625 == sliced[3].id);
	...
	sliced[2].line = 0;
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_LINE_ORDER, 3);

	sliced[1].line = 8;
	sliced[2].line = 8;
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_LINE_ORDER, 2);
	...
	sliced[1].id = VBI3_SLICED_TELETEXT_B_625;
	sliced[1].line = 11;
	sliced[2].id = VBI3_SLICED_VBI_625;
	sliced[2].line = 9;
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_LINE_ORDER, 2);

	sliced[1].id = VBI3_SLICED_VBI_625;
	sliced[1].line = 11;
	sliced[2].id = VBI3_SLICED_TELETEXT_B_625;
	sliced[2].line = 9;
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_LINE_ORDER, 2);
	...
	assert (VBI3_SLICED_TELETEXT_B_625 == sliced[17 + 1].id);
	assert (313 + 8 == sliced[17 + 1].line);
	assert (VBI3_SLICED_TELETEXT_B_625 == sliced[17 + 2].id);
	assert (313 + 9 == sliced[17 + 2].line);

	sliced[17 + 1].line = 313 + 10;
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_LINE_ORDER, 17 + 2);
...
test_dvb_mux_cor_packet_overflow_checks (void)
{
	...
	mx.set_sliced (sliced, n_lines);
	mx.test_fail (VBI3_ERR_BUFFER_OVERFLOW,
...
test_dvb_mux_feed_no_callback_checks (void)
{
	...
	assert (FALSE == success);
	// XXX Later
	// assert (VBI3_ERR_NO_CALLBACK == errno);
...
test_dvb_mux_new_pid_checks	(void)
{
	...
	assert (NULL == vbi3_dvb_ts_mux_new (0x123456, NULL, NULL)); 
	assert (NULL == vbi3_dvb_ts_mux_new (UINT_MAX, NULL, NULL)); 
}
